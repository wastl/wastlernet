...
var device_ids;
function listDevices(result, errc, errm, ud) {
    device_ids = result.addrs;
    console.log(result.addrs);
}
function hasDevice(addr) {
    for (let i = 0; i < device_ids.length; i++) {
        if (addr === device_ids[i]) return true;
    }
    return false;
}
...
function scanCB(ev, res) {
    ...
    try {
        ...
        if (hasDevice(addr)) {
            // post data to MQTT
            pushToMQ("shelly/relais/flur_werkstatt/motion_sensor", JSON.stringify(postMessage));
        } else {
            console.log("stray BLE device: "+addr);
        }
...
function init() {
    ...
    Shelly.call("BLE.CloudRelay.List", null, listDevices, null);
